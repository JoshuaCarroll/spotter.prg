<?php
$con = mysqli_connect("localhost","root","Passw0rd", "SpotterDB");
// Open db SpotterDB
if($con === false){
...
else
{
    ...
    $query="CREATE TABLE BRoster(Number INT(3),Name VARCHAR(30),Position VARCHAR(8), INDEX (Number));"; 
    ...
    $handle = fopen($file,"r");
    // Create a table for the Bruins
    $query="CREATE TABLE ORoster(Number INT(3),Name VARCHAR(30),Position VARCHAR(8), INDEX (Number));"; 
    if ((mysqli_query($con,$query))!==false) //test for table
    {
        echo "table created <br>";    
    }
